...
DEFAULT_CHATML_CHAT_TEMPLATE = "{% for message in messages %}\n{{'<s>' + message['role'] + '\n' + message['content'] + '</s>' + '\n'}}{% if loop.last and add_generation_prompt %}{{'<s>assistant\n' }}{% endif %}{% endfor %}"
    ...
    def preprocess(samples):
        ...
        if 'messages' in samples:
            for conversation in samples["messages"]:
                try:
                    # 尝试应用 chat_template
                    processed = tokenizer.apply_chat_template(conversation, tokenize=False)
                    batch.append(processed)
                except Exception as e:
                    print(conversation)
                    print(f"Error processing conversation: {e}")
        elif 'question' in samples and 'answer' in samples:
            for question, answer in zip(samples['question'], samples['answer']):
                try:
                    ...
                    print(f"Error processing question and answer: {e}")
            ...
            for input_text, output_text in zip(samples['input'], samples['output']):
                try:
                    ...
                    print(f"Error processing input and output: {e}")
        else:
            print("No recognizable keys found in samples.")
